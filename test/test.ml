...
let test_concurrent _switch () =
  ...
  Log.await log1 (Fmt.str "(from base)\n%s: (run (shell A))\nA\n" root) >>= fun () ->
  let b2 = B.build builder context2 spec2 in
  Log.await log2 (Fmt.str "(from base)\n%s: (run (shell A))\nA\n" root) >>= fun () ->
...
let test_cancel _switch () =
  ...
  Log.await log (Fmt.str "(from base)\n%s: (run (shell Wait))\nWait\n" root) >>= fun () ->
...
let test_cancel_4 _switch () =
  ...
  Log.await log3 (Fmt.str "(from base)\n%s: (run (shell Wait))\nWait\n" root) >>= fun () ->
...
let test_cancel_5 _switch () =
  if Sys.win32 then Alcotest.(check reject "Cancel 5 hangs on Windows for an unknown reason." () ());
  with_config @@ fun ~src_dir ~store ~sandbox ~builder ->
  let spec = Spec.(stage ~from:"base" [ run "Wait" ]) in
  let r, set_r = Lwt.wait () in
